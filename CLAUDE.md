...
# Data quality — outlier scanning and surgical fixes
...
python scripts/scan_outliers.py --stdout           # Print to stdout
python scripts/scan_outliers.py --severity critical # Filter by severity
python scripts/scan_outliers.py --json             # JSON output

python scripts/fix_outliers.py                     # Dry-run known fixes (default)
python scripts/fix_outliers.py --fix --backup      # Apply known fixes with backup
python scripts/fix_outliers.py --auto-scan         # Dry-run auto-detected fixes
python scripts/fix_outliers.py --auto-scan --fix --backup  # Apply auto-fixes with backup
python scripts/fix_outliers.py --config data/fix_config.json --fix  # Apply custom config
```
...
├── scan_outliers.py          # ⑤ Data quality scanner — detects outliers across all kr_stocks/ JSONs
├── fix_outliers.py           # ⑥ Pinpoint fixer — surgical fixes to specific fields (dry-run default)
...
reports/
└── outlier_report_YYYYMMDD.md  # Generated by scan_outliers.py
...
- **Revenue account collision (CRITICAL — fixed Mar 2026):** DART batch API returns multiple revenue-related accounts for the same company/period (e.g., `매출액` + `이자수익` for Naver, `매출액` + `영업수익` for holding companies). `REVENUE_NAMES` **must be a tuple** (ordered), not a set. `extract_value()` **must use `pick_max=True`** for revenue extraction — always select the largest matching value because total revenue ≥ any sub-component. This bug caused Naver (035420) and 카카오 (035720) to store `이자수익` (interest income, ~100-500억) instead of `매출액` (total revenue, ~2-10조). **Never use Python sets for DART account name matching.** Insurance/holding companies legitimately have `revenue < op_profit` in DART key accounts (not a bug — different accounting structure).
- **Q4 derivation:** `Q4 = Annual − Q1 − Q2 − Q3` can produce incorrect values when consolidation scope changes mid-year. The pipeline auto-detects anomalies and fixes them using `fnlttSinglAcnt` endpoint's `thstrm_add_amount` (9-month cumulative). Remaining negative Q4 entries are nullified. **NAVER (035420) Q4 2020** is a known unfixable case (LINE deconsolidation) — real values are hardcoded in `fix_outliers.py` KNOWN_FIXES and must be re-applied after any pipeline re-run via `python scripts/fix_outliers.py --fix`.
- **Missing financials from batch API:** ~140+ companies had missing revenue/op_profit/net_income because the batch API (`fnlttMultiAcnt`) only returns standard "주요계정" names. Companies using `영업수익` (e.g., POSCO홀딩스, 카카오 after holding company conversion) or `연결당기순이익` (e.g., 현대자동차) were missing data. Fixed by falling back to `fnlttSinglAcntAll` (full financial statement API). The fallback also uses `pick_max` for revenue. ~70 small companies still have missing revenue where DART has no revenue account at all (SPACs, pre-revenue biotech).
...
#### Outlier Scanner: `scripts/scan_outliers.py`
- Read-only scan of all ~3,064 per-company JSONs
- Detects: absurd values (>1경), negative Q4 revenue, Q4 op>revenue, EPS sign mismatches, one-time gains, revenue YoY anomalies, data completeness gaps
- Financial companies auto-detected by sector/name keywords and exempted from "op > revenue" check
- Outputs markdown report grouped by severity (Critical/Warning/Info), limited to 50 rows per category
- No external dependencies (standard library only)

#### Pinpoint Fixer: `scripts/fix_outliers.py`
- Surgical fixes: reads JSON → modifies ONLY specified fields → writes back
- **Dry-run by default** — requires `--fix` flag to write
- Three fix sources: hardcoded `KNOWN_FIXES`, `--auto-scan` (auto-detects unambiguous issues), `--config` (external JSON)
- Auto-scan fixes: absurd values → nullify, negative Q4 → nullify, Q4 op>revenue → nullify, EPS sign mismatch → nullify
- Auto-scan does NOT fix: one-time gains, revenue continuity, data completeness (these are mostly legitimate)
- Idempotent, grouped I/O (all fixes for same company in one read-write), optional `--backup`
- **CRITICAL**: Never re-derive or re-fetch values. Only modify the specific fields specified in the fix config.

#### Mar 2026 Data Quality Fix Results
- 1,984 fixes applied across 1,058 company files
- 94 critical issues → 0 (absurd values, negative Q4, Q4 op>revenue all nullified)
- 1,891 EPS sign mismatches nullified
- Remaining 3,892 warnings are legitimate business events (M&A, cyclical losses, one-time gains)
...
5. `scan_outliers.py` — Generate data quality report
6. `fix_outliers.py --auto-scan --fix --backup` — Apply surgical fixes (review dry-run first)
