...
손익계산서(02_손익계산서_연결)에는 revenue가 있는 기업들의 revenue 데이터를 파싱합니다.

예: 한화오션 [042660]

출력: income_statement_revenue.json
- 각 기업의 분기별 revenue 데이터
"""
...
OUTPUT_FILE = "income_statement_revenue.json"
...
def extract_revenue_from_comprehensive(filepath, filename):
...
def extract_revenue_data_from_income_statement(filepath, filename, target_companies):
    """손익계산서에서 타겟 기업들의 revenue와 영업이익 데이터 추출"""
    results = {}
    ...
    for row in rows[1:]:
        ...
        # 타겟 기업만 처리 (None이면 모든 기업 처리)
        if target_companies is not None and stock_code not in target_companies:
            continue
...
def compile_history(data):
    ...
    for year in sorted(data.keys()):
        ...
        if annual_data and q3_acc_data:
            ...
            for m in ['revenue', 'op_profit']:
                val_ann = annual_data.get(m)
...
def main():
    ...
    for f in comprehensive_files:
        ...
        companies = extract_revenue_from_comprehensive(filepath, f)
        comprehensive_revenue_by_period[period] = companies

    # 2단계: 손익계산서에서 revenue 데이터 추출 (포괄손익에 없는 기업만)
    print("\n=== 2단계: 손익계산서에서 revenue 데이터 추출 ===")
    ...
    for f in income_files:
        ...
        # 손익계산서에서 모든 기업의 revenue 추출
        # (나중에 필터링)
        results = extract_revenue_data_from_income_statement(
            filepath, f,
            target_companies=None  # 일단 모든 기업
        )
    ...
    print(f"\n총 {len(final_output)}개 기업의 revenue 데이터 추출 완료")
